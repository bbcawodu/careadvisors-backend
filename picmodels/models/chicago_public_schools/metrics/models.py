...
from .services.read import filter_metrics_qset_by_id
...
class CPSMetricsSubmission(models.Model):
    # fields for MetricsSubmission model
    staff_member = models.ForeignKey(CPSStaff, on_delete=models.CASCADE)
    ...
    def return_values_dict(self):
        valuesdict = {
            "no_general_assis": self.no_general_assis,
        ...
        plan_stats = self.metricsplanstatistic_set.all()
        if len(plan_stats):
            plan_stats_list = []
...
CPSMetricsSubmission.retrieve_metrics_data_by_id = classmethod(retrieve_metrics_data_by_id)
CPSMetricsSubmission.filter_metrics_qset_by_id = classmethod(filter_metrics_qset_by_id)
CPSMetricsSubmission.retrieve_metrics_data_by_staff_id = classmethod(retrieve_metrics_data_by_staff_id)
CPSMetricsSubmission.retrieve_metrics_data_by_staff_f_and_l_name = classmethod(retrieve_metrics_data_by_staff_f_and_l_name)
CPSMetricsSubmission.retrieve_metrics_data_by_staff_first_name = classmethod(retrieve_metrics_data_by_staff_first_name)
CPSMetricsSubmission.retrieve_metrics_data_by_staff_last_name = classmethod(retrieve_metrics_data_by_staff_last_name)
CPSMetricsSubmission.retrieve_metrics_data_by_staff_email = classmethod(retrieve_metrics_data_by_staff_email)


class MetricsPlanStatistic(models.Model):
    # Why is using variables in addition to touples better? Error prevention and logic separation.
    ...
    cps_metrics_submissions = models.ManyToManyField(CPSMetricsSubmission, blank=True)
