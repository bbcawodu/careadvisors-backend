def retrieve_staff_data_by_id(cls, rqst_staff_id, list_of_ids, rqst_errors):
    staff_qset = filter_staff_qset_by_id(cls.objects.all(), rqst_staff_id, list_of_ids)
...
def retrieve_staff_data_by_f_and_l_name(cls, rqst_first_name, rqst_last_name, rqst_errors):
    staff_qset = filter_staff_objs_by_f_and_l_name(cls.objects.all(), rqst_first_name, rqst_last_name)
...
def retrieve_staff_data_by_email(cls, list_of_emails, rqst_errors):
    response_list = []

    for email in list_of_emails:
        filtered_staff_qset = filter_staff_objs_by_email(cls.objects.all(), email)
...
def retrieve_staff_data_by_first_name(cls, list_of_first_names, rqst_errors):
    response_list = []

    for first_name in list_of_first_names:
        filtered_staff_qset = filter_staff_objs_by_first_name(cls.objects.all(), first_name)
...
def retrieve_staff_data_by_last_name(cls, list_of_last_names, rqst_errors):
    response_list = []

    for last_name in list_of_last_names:
        filtered_staff_qset = filter_staff_objs_by_last_name(cls.objects.all(), last_name)
...
def retrieve_staff_data_by_county(cls, list_of_counties, rqst_errors):
    response_list = []

    for county in list_of_counties:
        filtered_staff_qset = filter_staff_objs_by_county(cls.objects.all(), county)
...
def retrieve_staff_data_by_region(cls, list_of_regions, rqst_errors):
    response_list = []
...
def retrieve_staff_data_by_mpn(cls, list_of_mpns, rqst_errors):
    response_list = []

    for mpn in list_of_mpns:
        filtered_staff_qset = filter_staff_objs_by_mpn(cls.objects.all(), mpn)
...
def filter_staff_qset_by_id(db_queryset, rqst_id, list_of_ids):
...
def filter_staff_objs_by_f_and_l_name(db_queryset, rqst_first_name, rqst_last_name):
    db_queryset = prefetch_related_rows(db_queryset)
...
def filter_staff_objs_by_first_name(db_queryset, rqst_first_name):
    db_queryset = prefetch_related_rows(db_queryset)
...
def filter_staff_objs_by_last_name(db_queryset, rqst_last_name):
    db_queryset = prefetch_related_rows(db_queryset)
...
def filter_staff_objs_by_email(db_queryset, rqst_email):
    db_queryset = prefetch_related_rows(db_queryset)
...
def filter_staff_objs_by_county(db_queryset, rqst_county):
    db_queryset = prefetch_related_rows(db_queryset)
...
def filter_staff_objs_by_mpn(db_queryset, rqst_mpn):
    db_queryset = prefetch_related_rows(db_queryset)
